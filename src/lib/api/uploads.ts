...
async function uploadBranchAssetPhotoRequest(
  file: File,
  _retried = false,
): Promise<BranchAssetPhotoUploadResult> {
  if (!isLoginRoute() && (tokenStore.access || tokenStore.refresh)) {
    const fresh = await ensureFreshAccessToken();
    if (!fresh) {
      clearSessionAndRedirectToLogin();
  ...
  const res = await fetch(
    `${config.apiBaseUrl}/api/admin/uploads/branch-asset-photo`,
    {
      method: "POST",
      headers,
      body: formData,
  ...
  if (res.status === 401 && !_retried && !isLoginRoute()) {
    const refreshed = await performSessionRefresh();
    if (refreshed) {
      return uploadBranchAssetPhotoRequest(file, true);
    }
    clearSessionAndRedirectToLogin();
  }

  let json: Envelope<BranchAssetPhotoUploadResult>;
  try {
    json = (await res.json()) as Envelope<BranchAssetPhotoUploadResult>;
  } catch {
    throw new ApiError(res.status, "invalid_response", res.statusText);
  }
  ...
  return json.data as BranchAssetPhotoUploadResult;
}

/** Upload a branch asset photo and return the hosted URL for `photo_url`. */
export async function uploadBranchAssetPhoto(
  file: File,
): Promise<BranchAssetPhotoUploadResult> {
  const validationError = validateMenuPhotoFile(file);
  if (validationError) {
...
export type StaffKtpPhotoUploadResult = PhotoUploadResult;

async function uploadStaffKtpPhotoRequest(
  file: File,
  _retried = false,
): Promise<StaffKtpPhotoUploadResult> {
  if (!isLoginRoute() && (tokenStore.access || tokenStore.refresh)) {
    const fresh = await ensureFreshAccessToken();
    if (!fresh) {
      clearSessionAndRedirectToLogin();
  ...
  const res = await fetch(
    `${config.apiBaseUrl}/api/admin/uploads/staff-ktp-photo`,
    {
      method: "POST",
      headers,
      body: formData,
  ...
  if (res.status === 401 && !_retried && !isLoginRoute()) {
    const refreshed = await performSessionRefresh();
    if (refreshed) {
      return uploadStaffKtpPhotoRequest(file, true);
    }
    clearSessionAndRedirectToLogin();
  }

  let json: Envelope<StaffKtpPhotoUploadResult>;
  try {
    json = (await res.json()) as Envelope<StaffKtpPhotoUploadResult>;
  } catch {
    throw new ApiError(res.status, "invalid_response", res.statusText);
  }
  ...
  return json.data as StaffKtpPhotoUploadResult;
}

/** Upload a staff KTP photo and return the hosted URL for `ktp_photo_url`. */
export async function uploadStaffKtpPhoto(
  file: File,
): Promise<StaffKtpPhotoUploadResult> {
  const validationError = validateMenuPhotoFile(file);
  if (validationError) {
  ...
  return uploadStaffKtpPhotoRequest(file);
}
