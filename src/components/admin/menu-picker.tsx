...
export function formatMenuOptionLabel(menu: Pick<Menu, "title" | "category_name">) {
  return `${menu.title} · ${menu.category_name}`;
}
...
export interface MenuPickerProps {
  ...
  label?: string;
  value: string;
  selectedMenu?: Pick<Menu, "id" | "title" | "category_name"> | null;
...
export function MenuPicker({
  ...
  const loadOptions = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const result = await menusAdminApi.list({
        page: 1,
  ...
  const displayLabel = selectedMenu ? formatMenuOptionLabel(selectedMenu) : "";
  ...
  return (
    <div className="space-y-1.5" ref={containerRef}>
      ...
      <div className="relative">
        ...
        >
          <span className="truncate">{displayLabel || "Select a menu"}</span>
