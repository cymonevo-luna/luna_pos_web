#!/usr/bin/env node
/**
 * Browser verification for POS-144-11: admin disposal date edit.
 *
...
const MANAGER_USER_ID = "user-manager-verify-144-11";
const ADMIN_USER_ID = "user-admin-verify-144-11";
...
async function installApiMocks(page) {
  await page.route(`${API_BASE}/**`, async (route) => {
    ...
    const patchMatch = pathname.match(
      /^\/api\/admin\/menu-disposals\/([^/]+)\/record-date$/,
    );
    if (patchMatch && method === "PATCH") {
      const id = patchMatch[1];
      const body = request.postDataJSON();
      const item = allDisposals.find((d) => d.id === id);
      if (!item) {
        return route.fulfill(json({ success: false, error: "not_found" }, 404));
      }
      item.disposed_at = body.disposed_at;
...
async function main() {
  ...
  try {
    ...
    allDisposals[0].disposed_at = new Date().toISOString();
    ...
    const editVisible =
      (await page.getByTestId("menu-disposal-edit-date-disposal-verify-1").count()) >
      0;
    results.push([
      "Admin sees disposal date edit",
      editVisible ? "PASS" : "FAIL",
      "Edit date action visible on disposal record",
    ]);

    const lastMonth = lastMonthDateInput();
    await page.getByTestId("menu-disposal-edit-date-disposal-verify-1").click();
    await page.getByTestId("menu-disposal-edit-date-dialog").waitFor();
    await page.getByTestId("menu-disposal-edit-date-input").fill(lastMonth);
    await page.getByTestId("menu-disposal-edit-date-save").click();
    await page.getByTestId("menu-disposal-edit-date-dialog").waitFor({
      state: "hidden",
      timeout: 15000,
    });
    const updatedIso = dateInputToIso(lastMonth, false);
    const savedCorrectly = allDisposals[0].disposed_at === updatedIso;
    results.push([
      "Admin saves corrected disposal date",
      savedCorrectly ? "PASS" : "FAIL",
      `Disposed_at updated to ${allDisposals[0].disposed_at}`,
    ]);
    ...
    const managerEditHidden =
      ...
      0;
    results.push([
      "Non-admin cannot edit disposal date",
      managerEditHidden ? "PASS" : "FAIL",
      "Manager does not see edit date action",
    ]);
    ...
    await page.getByTestId("menu-disposals-page").waitFor({ timeout: 15000 });
    await page.getByTestId("menu-disposals-date-from").fill(lastMonth);
    await page.getByTestId("menu-disposals-date-to").fill(lastMonth);
    await page.getByText("Nasi Goreng").waitFor({ timeout: 15000 });
    results.push([
      "Date filter reflects correction",
      "PASS",
      `Record appears when filtering to ${lastMonth}`,
    ]);
