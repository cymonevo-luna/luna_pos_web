#!/usr/bin/env bash
# POS-60-3 / POS-63-2 live QA verification — admin-only production request detail.
#
# Prerequisites:
#   - luna_pos_service running (API :8087) for live browser checks
#   - Next.js dev server on :3000 with NEXT_PUBLIC_API_URL matching API_URL
#
# Usage:
#   ./scripts/verify-pos-60-3-qa.sh
#   NEXT_PUBLIC_API_URL=http://localhost:8087 WEB_BASE=http://localhost:3000 ./scripts/verify-pos-60-3-qa.sh
set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "$0")" && pwd)"
REPO_DIR="$(cd -- "$SCRIPT_DIR/.." && pwd)"
if [ -d "$REPO_DIR/../luna_pos_service" ]; then
...
bootstrap_api() {
	...
	if [ -d "$SERVICE_DIR" ]; then
		echo "   API down — attempting make docker-up-d in sibling luna_pos_service"
		(
			cd "$SERVICE_DIR"
			make docker-up-d
			if [ -x "$SERVICE_DIR/scripts/docker-wait-healthy.sh" ]; then
				"$SERVICE_DIR/scripts/docker-wait-healthy.sh"
			fi
		) >/tmp/pos-60-3-docker.log 2>&1 || true
	fi
	api_healthy
}
...
if MOCK_API=0 NEXT_PUBLIC_API_URL="$CLOSED_API_URL" WEB_BASE="$WEB_BASE" node "$SCRIPT_DIR/verify-pos-60-3-browser.mjs" >/tmp/pos-60-3-preflight.log 2>&1; then
	...
	if [ "$ELAPSED" -gt 8 ]; then
		fail "api_preflight" "took ${ELAPSED}s — expected fast failure within ~5s"
...
echo ">> [4/10] Mocked browser verification regression"
if web_healthy; then
	ensure_playwright
	if MOCK_API=1 NEXT_PUBLIC_API_URL="$API_URL" WEB_BASE="$WEB_BASE" node "$SCRIPT_DIR/verify-pos-60-3-browser.mjs" >/tmp/pos-60-3-mocked-browser.log 2>&1; then
		if grep -c "^PASS:" /tmp/pos-60-3-mocked-browser.log | grep -q '^6$'; then
			pass "mocked_browser" "six mocked PASS lines + final success message"
		else
			pass "mocked_browser" "mocked browser script exit 0"
		fi
	else
...
if bootstrap_api; then
	...
	mark_live_fail "API unreachable"
fi
...
echo ">> [7/10] Live browser smoke"
if ! api_healthy; then
	:
elif ! web_healthy; then
	mark_live_fail "dev server not running at $WEB_BASE (start with NEXT_PUBLIC_API_URL=$API_URL npm run dev)"
else
	ensure_playwright
	if MOCK_API=0 NEXT_PUBLIC_API_URL="$API_URL" WEB_BASE="$WEB_BASE" node "$SCRIPT_DIR/verify-pos-60-3-browser.mjs" >/tmp/pos-60-3-live-browser.log 2>&1; then
